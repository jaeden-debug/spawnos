...
function staticToCMS(s: SpeciesData): CMSSpecies {
  ...
  return {
    ...
    temperament: (s.care.temperament || 'peaceful') as CMSSpecies['temperament'],
    lifespan_years_min: null,
    lifespan_years_max: null,
    adult_size_cm_min: null,
    adult_size_cm_max: null,
    min_tank_litres: s.care.minTankGallons ? Math.round(s.care.minTankGallons * 3.785) : null,
    origin_regions: [s.origin],
    origin_countries: [],
    param_temp: { min: parseFloat(p.temperature.split('–')[0] ?? '24'), max: parseFloat(p.temperature.split('–')[1] ?? '28'), unit: '°C' },
    param_ph: { min: parseFloat(p.ph.split('–')[0] ?? '6.5'), max: parseFloat(p.ph.split('–')[1] ?? '7.5'), unit: 'pH' },
    param_gh: p.gh ? { min: parseFloat(String(p.gh).split('–')[0] ?? '0'), max: parseFloat(String(p.gh).split('–')[1] ?? '15'), unit: 'dGH' } : null,
    param_kh: p.kh ? { min: parseFloat(String(p.kh).split('–')[0] ?? '0'), max: parseFloat(String(p.kh).split('–')[1] ?? '10'), unit: 'dKH' } : null,
    param_tds: p.tds ? { min: parseFloat(String(p.tds).split('–')[0] ?? '0'), max: parseFloat(String(p.tds).split('–')[1] ?? '300'), unit: 'ppm' } : null,
    ...
    param_flow: null,
    param_lighting: null,
    care_substrate: s.care.substrate ? [s.care.substrate] : [],
    care_plants: s.care.plants ? s.care.plants.split(', ') : [],
    care_filtration: s.care.filtration ?? null,
    care_diet: s.care.diet ? s.care.diet.split(', ') : [],
    care_feeding_freq: null,
    care_social: null,
    compatible_with: s.compatibleWith ?? [],
    incompatible_with: s.incompatibleWith ?? [],
